public class HeapSort {
     ...
     * @param n    表示前面的数组是最大堆（改良版堆排序中，用到了原地的排序，原理是数组传进来一整个变成了最大堆，然后此时最左边的元素是最大的
     *             ，让其放到最右边，然后除去最后一个（也就是最右边的那个）剩下的数组变成最大堆继续执行上一步操作。
     * @param <E>  支持泛型
     */
    // 对data[0,n)所形成的最大堆中，索引 k 的元素，执行siftDown
    private static <E extends Comparable<E>> void siftDown(E[] data, int k, int n) {

        // 因为右孩子的索引比左孩子的索引还要大，如果左孩子的索引(2*k+1)都越界(>n的时候)了，右孩子的索引一定越界，
        // 也就是到了k是个叶子节点的时候了
        while (2 * k + 1 < n) {
